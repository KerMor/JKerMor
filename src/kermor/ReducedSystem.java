...
public class ReducedSystem implements FirstOrderDifferentialEquations {
	 ...
	 */
	public static ReducedSystem load(AModelManager mng) throws MathReaderException, ModelManagerException, IOException {
		...
		if ("dscomponents.LinearOutputConv".equals(mng.getModelXMLTagValue("outputconvtype"))) {
			res.C = new LinearOutputConv(r.readMatrix(mng.getInStream("C.bin")));
		}

		if ("dscomponents.ConstInitialValue".equals(mng.getModelXMLTagValue("initialvaluetype"))) {
	...
	private static void loadInputs(AModelManager mng, ReducedSystem res) throws IOException, ModelManagerException {
		// Inputs
		MathObjectReader r = mng.getMathObjReader();
		String hlp;
		if (mng.xmlTagExists("kermor_model.inputconv")) {
			...
			if ("dscomponents.LinearInputConv".equals(hlp)) {
				...
				AffParamMatrix a = new AffParamMatrix(r.readMatrix(mng.getInStream("B.bin")), res.dim, coeff);
				res.B = new AffineInputConv(a);
			}
		}
	}

	private static void loadCoreFun(AModelManager mng, ReducedSystem res) throws IOException, MathReaderException,
		...
		if ("dscomponents.ParamTimeKernelCoreFun".equals(type)) {
			...
			AffParamMatrix a = new AffParamMatrix(r.readMatrix(mng.getInStream("A.bin")), res.dim, coeff);
	...
	private static void loadMassMatrix(AModelManager mng, ReducedSystem res) throws IOException, ModelManagerException {
		// Inputs
		MathObjectReader r = mng.getMathObjReader();
		String hlp;
		if (mng.xmlTagExists("kermor_model.massmatrix")) {
			...
			if ("dscomponents.ConstMassMatrix".equals(hlp)) {
				res.M = new ConstMassMatrix(r.readMatrix(mng.getInStream("M.bin")));
			} else if ("dscomponents.AffLinMassMatrix".equals(hlp)) {
				IAffineCoefficients coeff = (IAffineCoefficients) mng.loadModelClass(mng
						.getModelXMLTagValue("massmatrix.coeffclass"));
				AffParamMatrix a = new AffParamMatrix(r.readMatrix(mng.getInStream("B.bin")), res.dim, coeff);
				res.B = new AffineInputConv(a);
	...
	private static IKernel loadKernel(AModelManager mng, String typestr, String datafile) throws MathReaderException,
			IOException {
		IKernel res = null;
